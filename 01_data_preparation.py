...
lipid_files_info = {
    'Chinese': {'file': os.path.join(BASE_DATA_PATH, 'Lipidomic', '122Chinese_282lipids.txt'),
                'fam': os.path.join(BASE_DATA_PATH, 'Genomics', '110Chinese_2527458snps.fam')},
    'Malay': {'file': os.path.join(BASE_DATA_PATH, 'Lipidomic', '117Malay_282lipids.txt'),
              'fam': os.path.join(BASE_DATA_PATH, 'Genomics', '108Malay_2527458snps.fam')},
    'Indian': {'file': os.path.join(BASE_DATA_PATH, 'Lipidomic', '120Indian_282lipids.txt'),
...
questionnaire_file = os.path.join(BASE_DATA_PATH, 'Phenotype', 'IOmics_Questionnaire_HealthScreening_and_FFQ.csv')
...
for ethnic_group, info in lipid_files_info.items():
    file_path = info['file']
    try:
        df = pd.read_csv(file_path, sep='\t', index_col=0) # Samples as rows, Lipids as columns
        ...
        if not available_lipid_columns: # Get columns from the first file
            available_lipid_columns.update(col for col in df.columns if col != 'ethnic_group')
        else: # Intersect to find common columns, though R script implies they are the same
            available_lipid_columns.intersection_update(col for col in df.columns if col != 'ethnic_group')
        print(f"Loaded {ethnic_group} lipid data: {df.shape[0]} samples, {len(df.columns)-1} lipids.")
...
if not available_lipid_columns:
    print("CRITICAL ERROR: No common lipid columns found across files or no files loaded properly.")
    exit()
...
if TARGET_LIPID_NAME_FROM_FILE is None:
    ...
    if available_lipid_columns:
        TARGET_LIPID_NAME_FROM_FILE = list(available_lipid_columns)[0] # Pick the first one
...
if TARGET_LIPID_NAME_FROM_FILE not in merged_lipid_df.columns:
    print(f"CRITICAL ERROR: Specified/chosen TARGET_LIPID_NAME_FROM_FILE ('{TARGET_LIPID_NAME_FROM_FILE}') not in merged data.")
    exit()
...
merged_lipid_df[TARGET_PHENOTYPE_COL] = pd.to_numeric(merged_lipid_df[TARGET_LIPID_NAME_FROM_FILE], errors='coerce')
...
if nan_count_before_impute > 0:
    ...
    print(f"Imputed {nan_count_before_impute} NaNs in '{TARGET_PHENOTYPE_COL}' with mean ({mean_pheno:.4f}).")

scaler = StandardScaler()
scaled_phenotype_col_name = f'{TARGET_PHENOTYPE_COL}_scaled'
merged_lipid_df[scaled_phenotype_col_name] = scaler.fit_transform(merged_lipid_df[[TARGET_PHENOTYPE_COL]])
print(f"Target phenotype '{TARGET_PHENOTYPE_COL}' (from '{TARGET_LIPID_NAME_FROM_FILE}') has been scaled into '{scaled_phenotype_col_name}'.")
...
try:
    ...
    if not (ID_COL_FOR_LIPID_DATA in id_map_df.columns and ID_COL_FOR_GENOMIC_DATA in id_map_df.columns):
        print(f"ERROR: Expected ID columns ('{ID_COL_FOR_LIPID_DATA}', '{ID_COL_FOR_GENOMIC_DATA}') not in {sample_id_mapping_file}.")
        print(f"Available columns: {id_map_df.columns.tolist()}")
        print("Proceeding without ID mapping from this file, which might lead to incorrect sample matching.")
        id_map_df = None # Invalidate map if columns are missing
except FileNotFoundError:
    print(f"ERROR: Sample ID mapping file not found: {sample_id_mapping_file}. ID mapping will be problematic.")
    id_map_df = None
except Exception as e:
...
if id_map_df is not None:
    pheno_mapped_df = pd.merge(merged_lipid_df, id_map_df[[ID_COL_FOR_LIPID_DATA, ID_COL_FOR_GENOMIC_DATA]],
                               left_on='SampleID_Lipid', right_on=ID_COL_FOR_LIPID_DATA, how='inner')
    pheno_mapped_df.rename(columns={ID_COL_FOR_GENOMIC_DATA: 'IID'}, inplace=True)
    if pheno_mapped_df.empty:
        print("CRITICAL ERROR: No samples common between lipid data and ID mapping file based on provided columns. Check ID column names and file contents.")
        exit()
    print(f"After merging with ID map: {pheno_mapped_df.shape[0]} samples remain.")
else:
    print("WARNING: ID mapping file not used or failed to load. Attempting to use lipid SampleIDs directly as IIDs.")
    print("This is unlikely to match genomic data unless lipid SampleIDs are identical to FAM IIDs.")
    pheno_mapped_df = merged_lipid_df.copy()
    pheno_mapped_df.rename(columns={'SampleID_Lipid': 'IID'}, inplace=True)

pheno_mapped_df['FID'] = pheno_mapped_df['IID'] # PLINK convention if no family structure
...
for ethnic_group, info in lipid_files_info.items():
    ...
    try:
        fam_df = pd.read_csv(fam_file_path, sep='\s+', header=None, usecols=[0, 1], names=['FID_fam', 'IID_fam'], dtype=str)
        # Crucial: Ensure IIDs are strings for merging and that FID/IID from FAM is used for final output FID/IID
        group_pheno_df['IID'] = group_pheno_df['IID'].astype(str) # Ensure IID from pheno is string
        
        # Merge with FAM to get FID_fam and ensure only samples in FAM are included
        final_group_pheno_df = pd.merge(group_pheno_df, fam_df, left_on='IID', right_on='IID_fam', how='inner')
        
        if final_group_pheno_df.empty:
            print(f"No common samples between mapped phenotypes and FAM file for {ethnic_group}. Skipping.")
            continue

        # Use FID from FAM file, and the scaled phenotype
        plink_output_df = final_group_pheno_df[['FID_fam', 'IID', scaled_phenotype_col_name]]
        plink_output_df.columns = ['FID', 'IID', TARGET_PHENOTYPE_COL] # Use consistent pheno name for PLINK
        ...
        print(f"Saved PLINK phenotype file for {ethnic_group} to {output_path} ({plink_output_df.shape[0]} samples)")
...
if all_pheno_for_plink_list:
    combined_pheno_for_plink = pd.concat(all_pheno_for_plink_list).drop_duplicates(subset=['FID', 'IID'])
    combined_output_path = os.path.join(output_dir, 'merged_pheno.txt')
    combined_pheno_for_plink.to_csv(combined_output_path, sep='\t', index=False, header=True)
    print(f"\nSaved combined PLINK phenotype file to {combined_output_path} ({combined_pheno_for_plink.shape[0]} unique samples)")
else:
    print("CRITICAL: No per-ethnic group phenotype files were generated. Cannot create combined file.")
...
if all_pheno_for_plink_list: # If combined_pheno_for_plink was created
    ...
    print("Cannot initialize covariate file as no combined phenotype data is available.")

print(f"\n--- Data Preparation Script Finished ---")
print(f"Target phenotype for PLINK: '{TARGET_PHENOTYPE_COL}' (derived from lipid '{TARGET_LIPID_NAME_FROM_FILE}')")
print(f"Scaled column name: '{scaled_phenotype_col_name}' (this is what's in the pheno files, but PLINK uses header name)")